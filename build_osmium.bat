...
if "%COMPILER%" == "MINGW" (
   ...
   cmake .. %GENERATOR% -DCMAKE_BUILD_TYPE=%VARIANT% -DCMAKE_INSTALL_PREFIX=%PREFIX% -DBOOST_ROOT=%BOOST_ROOT% -DBoost_USE_STATIC_LIBS=ON -DBUILD_TESTING=%TESTS% -DBUILD_HEADERS=%BUILD_HEADERS% -DBUILD_BENCHMARKS=%BUILD_BENCHMARKS%  -DBoost_ADDITIONAL_VERSIONS=1.58;1.59;1.60 -DBUILD_DATA_TESTS=%DATATEST%
   %ER%
   %MSBUILDINSTALL%
   %ER%
)
