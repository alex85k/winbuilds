...
if "%COMPILER%" == "MINGW" (
   ...
   cmake .. %GENERATOR% -DCMAKE_BUILD_TYPE=%VARIANT% -DCMAKE_INSTALL_PREFIX=%PREFIX% -DBOOST_ROOT=%BOOST_ROOT% -DBoost_USE_STATIC_LIBS=ON -DBUILD_TESTING=%TESTS% -DBUILD_TRY_HPPS=%ALLHPPS%  -DBoost_ADDITIONAL_VERSIONS=1.58;1.59 -DBUILD_DATA_TESTS=%DATATEST%
   %ER%
   %MSBUILDINSTALL%
   %ER%
)
